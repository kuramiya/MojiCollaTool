...
### 実装メモ
exeの隣に、MojiDataというフォルダを作成する。
保存するデータは一般的な文字データと同じとする。
保存時は、テキストを保存名と同じにする。
文字画面に保存ボタン、読み出しコントロールを追加する。
読み出し、保存コントロールをどのようにするか？

#### 保存処理
保存ボタン→保存ダイアログ（保存先は指定）→文字データ保存

#### 読み出し処理
読み出しボタン→読み出しダイアログ（読み出し場所は固定）→文字データ読み出し
or
選択コンボボックス→文字データ読み出し
